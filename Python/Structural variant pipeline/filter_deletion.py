#!/usr/bin/env python3
"""
Script Name: filter_deletion.py
...
Usage:
python3 <filter_deletion> --vcf_input <vcf_file> 
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Filters variants that are deletions"
        )
...
def main():
    ...
    def filter_deletion(vcf_input):
        """
        Filters variants to include only those that are deletions
        Index the output vcf if gzipped
        """

        # Compute output filename based on input filename
        in_path = Path(vcf_input)
        out_path = output_dir / f"{in_path.stem}_filter_deletions.vcf.gz"
        out_format = "z" if out_path.suffix == ".gz" else "v"

        # Bcftools command
        cmd = [
            "bcftools", "view",
            "--include", '(INFO/vep_VARIANT_CLASS = "deletion" || INFO/SVTYPE = "DEL" || SVTYPE = "DEL" || ALT = "<DEL>")',
            "--output", str(out_path),
    ...
    for vcf_file in vcf_input.split(","):
        ...
        output_vcf = filter_deletion(vcf_file)

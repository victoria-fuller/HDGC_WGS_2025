#!/usr/bin/env python3
"""
Script Name: combine_cnv_vcf.py
Author: Victoria Fuller
Date Created: 26/11/2025
Last Modified 26/11/2025

Description: 
Combines copy number VCF files generated by cnvkit into a multi-sample VCF

Usage:
python3 <combine_cnv_vcf.py> 
...
def main():
    ...
    if args.vcf_input:
        ...
        if not detected:
            ...
            logging.info(f"No input VCF provided - combining all VCFs")
    ...
    def compress_vcf_files(vcf_input_list):
        compressed_files = []

        for file in vcf_input_list:
            file_path = Path(file)
            compressed = output_dir / f"{file_path.name}.gz"
            
            logging.info(f"Compressing and indexing {file} -> {compressed}")

            # Compress files
            with open(compressed, "wb") as out_file:
                subprocess.run(["bgzip", "-c", str(file_path)], stdout=out_file, check=True)
    ...
    compressed_files = compress_vcf_files(vcf_input_list)

    def merge_vcf_files(compressed_files):
        """
        Finds all cnvkit VCF files in input directory and merges them using bcftools
        Indexes the merged vcf if gzipped
        """

        # Compute output filename based on input filename
        out_path = output_dir / "merged.cnvcall.vcf.gz"
        logging.info(f"Merging VCFs into {out_path}")
    ...
    output_vcf = merge_vcf_files(compressed_files)

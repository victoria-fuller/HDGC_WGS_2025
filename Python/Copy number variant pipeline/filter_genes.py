#!/usr/bin/env python3
"""
Script Name: filter_genes.py
Author: Victoria Fuller
Date Created: 20/11/2025
Last Modified 20/11/2025

Description: 
Description: Keeps variants with in or near to genes of interest
Cleans BioMart TSV file
Creates BED file for genes of interest using BioMart file
Filters VCF file by BED file

Usage:
python3 <filter_genes> --vcf_input <vcf_file> 
...
Notes to user:
User will need to modify the accompanying gene list file (gene_list.txt) to select required genes of interest
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Filters variants in or near to genes of interest"
        )
...
def main():
    ...
    def build_bed_file(biomart_file, gene_list, flank_bp):
        """Builds the BED file using the coordinates from the BioMart TSV file for genes of interest"""
        gene_list_path = Path(gene_list)
        bed_file = output_dir / f"{gene_list_path.stem}.bed"
        logging.info(f"Building BED file for genes in {gene_list}")

        with open(gene_list) as gene_file:
            genes = {line.strip() for line in gene_file if line.strip()}
            logging.info(f"Genes found: {', '.join(genes)}")
    ...
    bed_file = build_bed_file(biomart_file_cleaned, gene_list, flank_bp)
    ...
    def filter_vcf_by_bed(vcf_input, compressed_bed):
        """
        Filters variants using the BED file
        Index the output vcf if gzipped
        """

        # Compute output filename based on input filename
        in_path = Path(vcf_input)
        out_path = output_dir / f"{in_path.stem}_filter_genes.vcf.gz"
        out_format = "z" if out_path.suffix == ".gz" else "v"

        # Bcftools command
        cmd = [
            "bcftools", "view",
            "--regions-file", str(compressed_bed),
            "--output", str(out_path),
    ...
    for vcf_file in vcf_input.split(","):
        ...
        output_vcf = filter_vcf_by_bed(vcf_file, compressed_bed)

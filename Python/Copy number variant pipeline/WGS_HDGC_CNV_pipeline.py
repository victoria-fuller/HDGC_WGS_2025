#!/usr/bin/env python3
"""
Script Name: WGS_HDGC_CNV_pipeline.py
...
Usage:
python3 <WGS_HDGC_CNV_pipeline.py> --pipeline <pipeline> --vcf_input <vcf_file> --gene_list <gene_list> --biomart_file <biomart_file> --flank_bp <flank_bp>
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Automates WGS HDGC CNV filtering pipeline"
        )
...
def main():
    ...
    if args.vcf_input:
        ...
        detected = list(Path(input_dir).glob("*.cnvcall.vcf*"))
        if not detected:
    ...
    # Define pipelines
    PIPELINES = {
        "whole_genome_deletion": [
            "combine_cnv_vcf.py", 
            "cnv_annotate.py", 
            "cnv_filter_deletion.py", 
            "vcf_to_tsv.py"
            ],
        "genes_deletion": [
            "combine_cnv_vcf.py", 
    ...
    def run_script(script, current_file, extra_args=None):
        ...
        elif "combine_cnv_vcf" in script:

#!/usr/bin/env python3
"""
Script Name: filter_tfbs.py
Author: Victoria Fuller
Date Created: 20/11/2025
Last Modified 20/11/2025

Description: 
Description: Keeps variants with a MODIFIER impact that affect transcription factor binding sites

Usage:
python3 <filter_tfbs> --vcf_input <vcf_file> 
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Filters variants that affect transcription factor binding sites"
        )
...
def main():
    ...
    def filter_tfbs(vcf_input):
        """
        Filters variants to include only those with a modifier impact that affect transcription factor binding sites
        Index the output vcf if gzipped
        """

        # Compute output filename based on input filename
        in_path = Path(vcf_input)
        out_path = output_dir / f"{in_path.stem}_filter_tfbs.vcf.gz"
    ...
    for vcf_file in vcf_input.split(","):
        ...
        output_vcf = filter_tfbs(vcf_file)

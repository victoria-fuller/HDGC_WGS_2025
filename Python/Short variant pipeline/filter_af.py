#!/usr/bin/env python3
"""
Script Name: filter_af.py
Author: Victoria Fuller
Date Created: 19/11/2025
Last Modified 19/11/2025

Description: 
Description: Keeps variants that have an maximum allele frequency of < 0.01 or missing in gnomAD

Usage:
python3 <filter_af> --vcf_input <vcf_file> 
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Filters variants that have a maximum allele frequency in gnomAD < 0.01"
        )
...
def main():
    ...
    def filter_af(vcf_input):
        """
        Filters variants to include only those that have a maximum allele frequency of < 0.01 in gnomAD
        Index the output vcf if gzipped
        """

        # Compute output filename based on input filename
        in_path = Path(vcf_input)
        out_path = output_dir / f"{in_path.stem}_filter_af.vcf.gz"

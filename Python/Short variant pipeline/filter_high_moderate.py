#!/usr/bin/env python3
"""
Script Name: filter_high_moderate.py
Author: Victoria Fuller
Date Created: 19/11/2025
Last Modified 19/11/2025

Description: 
Description: Keeps variants with a HIGH impact or MODERATE impact with SIFT < 0.05 and PolyPhen > 0.908

Usage:
python3 <filter_high_moderate> --vcf_input <vcf_file> 
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Filters variants that have a high or moderate impact"
        )
...
def main():
    ...
    def filter_high_moderate(vcf_input):
        """
        Filters variants to include only those with a high or moderate impact
        Index the output vcf if gzipped
        """

        # Compute output filename based on input filename
        in_path = Path(vcf_input)
        out_path = output_dir / f"{in_path.stem}_filter_high_moderate.vcf.gz"
    ...
    for vcf_file in vcf_input.split(","):
        ...
        output_vcf = filter_high_moderate(vcf_file)

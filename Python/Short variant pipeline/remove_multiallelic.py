#!/usr/bin/env python3
"""
Script Name: remove_multiallelic.py
Author: Victoria Fuller
Date Created: 19/11/2025
Last Modified 19/11/2025

Description: 
Description: Removes multiallelic sites to prevent issues with bcftools +split-vep plugin
Usage:
python3 <remove_multiallelic> --vcf_input <vcf_file> 
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Filters to remove multiallelic sites"
        )
...
def main():
    ...
    def remove_multiallelic(vcf_input):
        """
        Filters variants to remove multiallelic sites
        Index the output vcf if gzipped
        """

        # Compute output filename based on input filename
        in_path = Path(vcf_input)
        out_path = output_dir / f"{in_path.stem}_remove_multiallelic.vcf.gz"
    ...
    for vcf_file in vcf_input.split(","):
        ...
        output_vcf = remove_multiallelic(vcf_file)

...
Usage:
python3 <genotype_qc> --vcf_input <vcf_file> 
...
def main():
    ...
    def genotype_qc(vcf_input):
        """
        Performs per sample genotype filtering and assigns missing (./.) to the genotypes that fail
        Index the output vcf if gzipped
        """

        # Compute output filename based on input filename
        in_path = Path(vcf_input)
        out_path = output_dir / f"{in_path.stem}_genotype_qc.vcf.gz"
    ...
    for vcf_file in vcf_input.split(","):
        ...
        output_vcf = genotype_qc(vcf_file)

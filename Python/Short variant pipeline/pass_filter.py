#!/usr/bin/env python3
"""
Script Name: pass_filter.py
Author: Victoria Fuller
Date Created: 19/11/2025
Last Modified 19/11/2025

Description: 
Description: Filters variants ot include only those that have PASS in the filter field from annotated VCF using bcftools
Removes variants VQSLOD less than the 99.9% truth sensitivity threshold 
Removes variants with missing genotypes (./.) or homozygous reference in the proband

Usage:
python3 <pass_filter> --vcf_input <vcf_file> 
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Filters variants that PASS quality filters"
        )
...
def main():
    ...
    def pass_filter(vcf_input):
        ...
        out_path = output_dir / f"{in_path.stem}_pass_filter.vcf.gz"
    ...
    for vcf_file in vcf_input.split(","):
        ...
        output_vcf = pass_filter(vcf_file)

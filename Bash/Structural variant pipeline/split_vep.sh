...
# Run: bash split_vep.sh <vcf file>
...
# Extract VEP annotation fields
vep_columns=$(zgrep "^##INFO=<ID=CSQ" $vcf_input_file|\
sed -E 's/.*Consequence annotations from Ensembl VEP. Format: //;s/">.*//' |\
tr '|' ',')

# Check if columns were extracted correctly 
if [[ -z "$vep_columns" ]]; then
  echo "ERROR: Could not extract VEP columns from $vcf_input_file"
  exit 1
fi
